...
const Write = () => {
  ...
  async function View_Detail() {
    ...
    if (type === 'DETAILCHAPTER') {
      ...
      let cmt =  localStorage.getItem('chapter_cmt')
      let cmt_content = 'No Comment'
      if(cmt !== '[]'){
        cmt_content = ( 'Comment of ' + JSON.parse(cmt)[0].EDITORID +  ': ' + JSON.parse(cmt)[0].CONTENT )
      }
      //let cmt = ( 'CMT of ' + data[0].EDITORID +  ': ' + data[0].CONTENT )
      displayCmt = <div className='bg-[#ffffff] w-[100%] h-fit rounded-lg ml-[200px] shadow-sm border-[2px]'>{cmt_content}</div>
    }
